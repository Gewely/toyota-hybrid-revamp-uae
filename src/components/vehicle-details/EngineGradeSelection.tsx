...
function monthlyPayment(price: number, opts: {
...
const Segmented: React.FC<{
  ...
  return <div role="radiogroup" aria-label={ariaLabel} className="relative inline-flex items-center rounded-3xl bg-white p-1 shadow-sm ring-1 ring-black/5">
      {options.map(opt => {
      const selected = value === opt.id;
      return <motion.button key={opt.id} role="radio" aria-checked={selected} onClick={() => onChange(opt.id)} className="relative rounded-2xl px-4 py-2 text-sm font-medium text-foreground" whileTap={{
        scale: 0.98
      }}>
            {/* animated pill background */}
            {selected && <motion.span layoutId="seg-bg" className="absolute inset-0 rounded-2xl" style={{
          background: "#EB0A1E"
        }} />}
  ...
  grade
    ...
    <CardContent className="p-0">
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        {grade.badge && <div className="absolute left-4 top-4">
...
const EngineGradeSelection: React.FC<EngineGradeSelectionProps> = ({
  ...
  const liveMonthly = (price: number) => monthlyPayment(price, {
    termMonths: term,
    downPaymentPct: dpPct,
    annualRate: apr
  });
  ...
  return <section className="bg-[linear-gradient(180deg,#FAFAFC_0%,#F4F6F8_100%)] py-12 lg:py-20">
      <div className="toyota-container">
        ...
        <div className="grid items-start gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            ...
            {isMobile ? <Carousel className="w-full">
                <CarouselContent>
                  {grades.map(g => <CarouselItem key={g.name}>
                      <GradeHero vehicleName={vehicle.name} grade={g} />
                    </CarouselItem>)}
                </CarouselContent>
                <CarouselPrevious className="left-2" />
                <CarouselNext className="right-2" />
              </Carousel> : <AnimatePresence mode="wait">
            ...
            <Card className="sticky top-20 rounded-3xl border-0 bg-white p-1 shadow-[0_16px_40px_rgba(0,0,0,0.07)]">
              <CardContent className="p-6">
                ...
                <div className="mb-4 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  ...
                  return <motion.button key={g.name} onClick={() => setActiveGradeName(g.name)} className="relative flex items-center gap-2 rounded-full border px-4 py-2 text-sm" whileTap={{
                    scale: 0.98
                  }}>
                ...
                <div className="mb-4 grid gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {[36, 48, 60].map(t => <Button key={t} variant={term === t ? "default" : "outline"} size="sm" className="rounded-full" onClick={() => setTerm(t as 36 | 48 | 60)}>
                  ...
                  <RangeControl label="Down payment" min={0.1} max={0.5} step={0.05} value={dpPct} onChange={setDpPct} format={v => `${Math.round(v * 100)}%`} />
                  <RangeControl label="APR" min={0.02} max={0.06} step={0.0025} value={apr} onChange={setApr} format={v => `${(v * 100).toFixed(2)}%`} />
                </div>
        ...
        {isMobile && <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 px-4 pb-[calc(env(safe-area-inset-bottom)+12px)]">
