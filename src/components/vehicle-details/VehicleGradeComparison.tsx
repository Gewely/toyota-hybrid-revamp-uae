...
import { X, Check, Download, Wrench, Car, Star, Eye, EyeOff } from "lucide-react";
        ...
        <div className="mt-4 lg:mt-6 space-y-4 lg:space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setShowOnlyDifferences(!showOnlyDifferences)} className="flex items-center gap-2 min-h-[44px]">
            ...
            <div className={`overflow-x-auto pb-2 ${isMobile ? '-mx-4 px-4' : ''}`}>
              <div className={`flex gap-3 ${isMobile ? 'min-w-max' : 'flex-wrap'}`}>
                {grades.map((grade, index) => <Button key={grade.name} variant={selectedGrades.includes(index) ? "default" : "outline"} size="sm" onClick={() => toggleGradeSelection(index)} className={`h-auto p-4 flex flex-col items-start min-h-[52px] ${isMobile ? 'min-w-[150px]' : ''} ${selectedGrades.includes(index) ? 'bg-[hsl(var(--toyota-graphite))] text-white border-[hsl(var(--toyota-graphite))] shadow-[0_4px_16px_rgba(0,0,0,0.15)]' : 'bg-white border-[hsl(var(--toyota-platinum))]/40 hover:border-[hsl(var(--toyota-graphite))]/30 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]'}`} disabled={selectedGrades.length >= maxSelectableGrades && !selectedGrades.includes(index)}>
                    <div className="flex items-center gap-2 mb-1">
                      {selectedGrades.includes(index) && <Check className="h-3 w-3" />}
                      <span className="font-bold text-sm">{grade.name}</span>
                      {grade.badge === "Most Popular" && <Badge className="bg-[hsl(var(--luxury-gold))]/20 text-[hsl(var(--luxury-gold))] border-[hsl(var(--luxury-gold))]/30 text-xs px-1.5 py-0.5">
          ...
          {selectedGrades.length > maxGradesInView && <div className="flex items-center justify-between mb-4">
              <Button variant="outline" size="sm" onClick={scrollPrev} disabled={!canScrollPrev} className="min-h-[40px]">
                Previous
              </Button>
              <span className="text-sm text-muted-foreground">
                Viewing {scrollOffset + 1}-{Math.min(scrollOffset + maxGradesInView, selectedGrades.length)} of {selectedGrades.length}
              </span>
              <Button variant="outline" size="sm" onClick={scrollNext} disabled={!canScrollNext} className="min-h-[40px]">
                Next
              </Button>
            </div>}
          
          <div className={`${isMobile ? 'grid grid-cols-2 gap-3' : `grid grid-cols-${visibleGrades.length} gap-6`}`}>
            {visibleGradeObjects.filter(grade => grade).map((grade, idx) => <Card key={grade.name} className="overflow-hidden border-[hsl(var(--toyota-platinum))]/30 shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition-all">
                <CardContent className="p-0">
                  <div className={`${isMobile ? 'aspect-[4/3]' : 'aspect-video'} overflow-hidden relative group`}>
                    <img src={grade.image} alt={grade.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  ...
                  <div className={`${isMobile ? 'p-4' : 'p-5 lg:p-6'} bg-gradient-to-br from-white to-[hsl(var(--neutral-50))]`}>
                    ...
                    <div className={`${isMobile ? 'space-y-2' : 'grid grid-cols-3 gap-2'}`}>
                      ...
                      <Button variant="outline" size="sm" className={`text-xs min-h-[40px] ${isMobile ? 'w-full' : ''}`} onClick={() => {
                    ...
                    onClose();
                      ...
                      <Button size="sm" className={`text-xs min-h-[40px] ${isMobile ? 'w-full' : ''}`} onClick={() => {
          ...
          <div className="space-y-3">
            {comparisonSpecs.map((section, index) => <PremiumComparisonSection key={section.title} title={section.title} items={section.items} grades={visibleGradeObjects} showOnlyDifferences={showOnlyDifferences} defaultOpen={!isMobile && index === 0} />)}
          </div>
