import { motion, useScroll, useSpring, useTransform } from 'framer-motion';
import { useEffect, useState } from 'react';

export const ScrollProgressBar = () => {
  const { scrollYProgress } = useScroll();
  const scaleX = useSpring(scrollYProgress, {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001
  });

  const [section, setSection] = useState(0);

  useEffect(() => {
    return scrollYProgress.on('change', (latest) => {
      const newSection = Math.floor(latest * 5);
      setSection(newSection);
    });
  }, [scrollYProgress]);

  const gradients = [
    'linear-gradient(90deg, hsl(var(--primary)) 0%, hsl(var(--primary-glow)) 100%)',
    'linear-gradient(90deg, hsl(var(--accent)) 0%, hsl(var(--primary)) 100%)',
    'linear-gradient(90deg, hsl(var(--secondary)) 0%, hsl(var(--accent)) 100%)',
    'linear-gradient(90deg, hsl(var(--primary-glow)) 0%, hsl(var(--secondary)) 100%)',
    'linear-gradient(90deg, hsl(var(--primary)) 0%, hsl(var(--accent)) 100%)',
  ];

  return (
    <motion.div
      className="fixed top-0 left-0 right-0 h-1 z-[9999] origin-left"
      style={{
        scaleX,
        background: gradients[section] || gradients[0],
      }}
    />
  );
};
